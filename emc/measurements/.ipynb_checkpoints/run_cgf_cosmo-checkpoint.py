...
if __name__ == '__main__':
    ...
    with MemoryMonitor() as mem:
        ...
        for j in range(args.start_cosmo, args.start_cosmo+args.n_cosmo):
            
            density_dir = '/pscratch/sd/m/mpinon/density/cosmo/r{:.0f}'.format(r)
            try:
                os.mkdir(density_dir)
                print(f"Directory '{density_dir}' created successfully.")
            except FileExistsError:
                print(f"Directory '{density_dir}' already exists.")
            density_dir = '/pscratch/sd/m/mpinon/density/cosmo/r{:.0f}/c{:03d}_ph000'.format(r, j)
            try:
                os.mkdir(density_dir)
                print(f"Directory '{density_dir}' created successfully.")
            except FileExistsError:
                print(f"Directory '{density_dir}' already exists.")
            data_dir = '/pscratch/sd/e/epaillas/emc/hods/cosmo+hod/z0.5/yuan23_prior/c{:03d}_ph000/seed0'.format(j)

            for i in range(args.start_hod, args.start_hod+args.n_hod):
                ...
                for los in ['x', 'y', 'z']:
                    ...
                    cgf_fn = Path(density_dir) / 'density_cgf_cellsize{}{}_hod{:03d}_los{}_logbins.npy'.format(cellsize, '_r{:.0f}'.format(r) if r else '', i, los)
